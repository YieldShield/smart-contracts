...
contract SplitRiskPoolCommissionTest is Test {
    ...
    /// @notice Test migration function for existing positions
    function testMigrationFunction() public {
        uint256 backingAmount = 1000e18;
        uint256 shieldedAmount = 500e18;
        ...
        uint256 rewardDebt = pool.rewardDebt(tokenId);
        assertEq(rewardDebt, 0, "Migrated position should have debt set to 0");

        // Step 4: Verify position can claim all accumulated rewards
        uint256 claimable = pool.getClaimableCommission(tokenId);
        assertTrue(claimable > 0, "Migrated position should be able to claim all accumulated rewards");
    }

    /// @notice Test that migration on position with non-zero debt reverts
    function testMigrationRevertsWithNonZeroDebt() public {
        uint256 backingAmount = 1000e18;
        uint256 shieldedAmount = 500e18;

        // Step 1: Deposit enough protector tokens for collateral (150% of shielded)
        vm.prank(protector1);
        pool.depositBackingAsset(address(backingToken), 1000e18, 0); // Enough for 500e18 shielded

        // Step 2: Generate some commissions first (so rewardPerShareAccumulated > 0)
        vm.prank(shielded);
        pool.depositShieldedAsset(address(shieldedToken), shieldedAmount, 0);
        oracle.setPrice(address(shieldedToken), 1.1e8);
        _claimRewardsAsOwner(0);

        // Step 3: Now create a new position (rewardDebt will be > 0)
        vm.prank(protector2);
        uint256 tokenId = pool.depositBackingAsset(address(backingToken), backingAmount, 0);
    ...
    /// @notice Test that non-governance cannot call migration
    function testOnlyGovernanceCanMigrate() public {
        uint256 backingAmount = 1000e18;

        vm.prank(protector1);
        uint256 tokenId = pool.depositBackingAsset(address(backingToken), backingAmount, 0);

        // Try to migrate as non-governance - should revert
        vm.prank(protector1);
        vm.expectRevert();
        pool.migrateExistingPosition(tokenId);
    }
