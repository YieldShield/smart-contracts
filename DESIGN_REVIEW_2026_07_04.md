...
- **All orchestration, review, adversarial verification, and synthesis in this document were performed by Claude Fable 5** (`claude-fable-5`) — the orchestrator and all reviewer/verifier subagents ran on that model.
- **No fallback to Claude Opus 4.8 was required.** Every subagent completed on the first attempt; no capacity, timeout, or capability event forced a downgrade.
- Web research (ERC-4626 manipulation, Chainlink bounds/sequencer, Pyth confidence, Uniswap V3 TWAP, UUPS upgrades, DAO governance capture) was performed by Claude Fable 5 via web search; sources are listed at the end.
